...
frozen gold mini-ontology (§11.3 de-risking slice), with every cell's prov_ref
interned over atoms minted from the ACTUAL source cells (same approach as
tests/integration/test_wave2_induction_seam.py).

The fixture-builder plays the role of the Wave-2/3 pipeline at gold-ontology
fidelity: conformance (currency lexical forms, meter-suffixed altitudes,
operator-name folding, leading-N normalization) happens HERE, with the source
lexical unit recorded alongside the conformed measure (altitude_agl_unit) —
exactly what ANVIL-conformed entity shards would contain. LODESTONE itself
never sees the CSVs.
"""
...
from ontoforge.contracts.ontology import Datatype, PropertyDef, property_uri
...
class WorldBuilder:
    ...
    def __init__(self, estate: dict, ledger: SqliteLedger, hearth: Hearth) -> None:
        self.estate = estate
        self.ledger = ledger
        self.hearth = hearth
    ...
    def commit_all(self) -> None:
        for cls in sorted(self.cells):
            self.hearth.commit(Layer.ENTITY, curi(cls), self.cells[cls])
        for (cls, pred) in sorted(self.links):
            self.hearth.commit_links(curi(cls), pred, self.links[(cls, pred)])
...
def build_world(estate, ledger: SqliteLedger, hearth: Hearth) -> WorldBuilder:
    b = WorldBuilder(estate, ledger, hearth)
    t = estate["tables"]
    ...
    for (comp, ata), atoms in sorted(comp_atoms.items()):
        curi_ = f"ent://component/{slug(comp + '|' + ata)}"
        b.put("Component", curi_, "component_name", comp, b.ref_sum(atoms))
        b.put("Component", curi_, "ata_chapter", ata, b.ref_sum(atoms))
...
@pytest.fixture(scope="session")
def gold_onto():
    return extend_ontology(load_gold_ontology())
...
def hearth_world(tmp_path_factory, estate, gold_onto, ledger):
    ...
    build_world(estate, ledger, hearth)
    return hearth
